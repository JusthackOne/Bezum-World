...
export class BossBattlesController {
  ...
  @Get() @ApiOperation({ summary: 'List boss battles' }) list() {
  ...
  @Get(':id/leaderboard') leaderboard(
    ...
    @Query() query: BossLeaderboardQueryDto,
    @Req() request: RequestWithAuthUser,
  ) {
    return this.service.leaderboard(params.id, this.userId(request), query.page, query.limit);
  }
  @Get(':id/me') me(@Param() params: BossBattleIdParamsDto, @Req() request: RequestWithAuthUser) {
    return this.service.get(params.id, this.userId(request));
  }
  @Get(':id/rewards') rewards(
    @Param() params: BossBattleIdParamsDto,
    @Req() request: RequestWithAuthUser,
  ) {
    return this.service.get(params.id, this.userId(request));
  }
  @Post(':id/attacks') attack(
    @Param() params: BossBattleIdParamsDto,
    @Req() request: RequestWithAuthUser,
  ) {
  ...
  @Post(':id/rewards/claim') claim(
    ...
    return this.service.claim(params.id, this.userId(request));
  }
  private userId(request: RequestWithAuthUser) {
    if (!request.user?.sub || request.user.actorType !== 'user')
      throw new ForbiddenException('Only active users can access boss battles');
    return request.user.sub;
  }
}

@ApiTags('admin-boss-battles')
@ApiBearerAuth('access-token')
@UseGuards(AccessTokenGuard, AdminOnlyGuard)
@Controller('admin/boss-battles')
export class AdminBossBattlesController {
  constructor(private readonly service: BossBattlesService) {}
  @Post() create(@Body() body: CreateBossBattleDto, @Req() request: RequestWithAuthUser) {
    return this.service.create(this.adminId(request), body);
  }
  @Post('images')
  @UseInterceptors(FileInterceptor('image', { limits: { fileSize: 5 * 1024 * 1024 } }))
  async uploadImage(@UploadedFile() file?: { mimetype: string; buffer: Buffer }) {
    ...
    const directory = join(process.cwd(), 'uploads', 'boss-battles');
    await mkdir(directory, { recursive: true });
    const fileName = `${randomUUID()}${extension}`;
    await writeFile(join(directory, fileName), file.buffer);
    return { imageUrl: `/uploads/boss-battles/${fileName}` };
  }
  @Get() list() {
    return this.service.list();
  }
  @Get(':id') get(@Param() params: BossBattleIdParamsDto) {
    return this.service.get(params.id);
  }
  @Patch(':id') update(
    @Param() params: BossBattleIdParamsDto,
    @Body() body: UpdateBossBattleDto,
    @Req() request: RequestWithAuthUser,
  ) {
    return this.service.update(this.adminId(request), params.id, body);
  }
