...
4. Use the visual map editor to create playable axial cells within axial radius 25. The `Add / remove hex` tool selects `Neutral`, `Team A`, or `Team B` as the owner of every newly added cell, including cells added during one drag stroke. Dedicated Team A/B ownership tools are not shown; `Ownership: neutral` remains available to clear an existing cell. `Mirror shape` treats the left half (`q < 0`) as authoritative and replaces the right half (`q > 0`) with its exact mirror across the vertical hex axis. Terrain and neutral ownership are preserved; captured/team-owned cells, buildings, towers, and spawn points are assigned to the opposite team. Cells and objects on the center axis (`q = 0`) remain unchanged. Paint ground/mountains, place one town hall and one distinct spawn per team, and configure resource-building ownership and income. Initial player placement is automatic.
