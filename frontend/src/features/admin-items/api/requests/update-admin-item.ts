...
import type { AdminItem, AdminUpdateItemInput } from "../../model/admin-item.types";

export async function updateAdminItem(payload: AdminUpdateItemInput): Promise<AdminItem> {
  ...
  return requestApiData(
    () =>
      adminHttpClient.patch<ApiSuccessResponse<AdminItem>>(
        `/admin/items/${encodeURIComponent(payload.itemId)}`,
        formData,
      ),
